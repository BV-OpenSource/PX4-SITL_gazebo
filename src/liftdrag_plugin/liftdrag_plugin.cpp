...
LiftDragPlugin::LiftDragPlugin() : cla(1.0), cda(0.01), cma(0.0), rho(1.2041)
{
    ...
    // Set debug velocity to 0
    this->debugVel = ignition::math::Vector3d(0.0, 0.0, 0.0);
}
                          ...
                          sdf::ElementPtr _sdf)
{
    ...
    if(this->debugPrint){
        gzdbg << "sensor \t link \t pose \t dynamic pressure \t spd \t vel \t LiftDrag Plane spd \t LiftDrag Plane vel \t forward \t upward \t lift dir \t"
              << " span dir \t angle \t cl \t lift \t drag \t cd \t cda \t moment \t force \n";
    }
}
...
void LiftDragPlugin::OnUpdate()
{
    ...
    if(this->debugVel.Length() != 0.0){
        vel = this->debugVel;
    }else{
        vel = this->link->WorldLinearVel(this->cp) - wind_vel_;
    }
    ...
    //    if(0){
    if (this->debugPrint){
        //        gzdbg << this->GetHandle() << "\t" << this->link->GetName()
        //              << "\t" << pose
        //              << "\t" << q
        //              << "\t" << vel.Length()
        //              << "\t" << vel
